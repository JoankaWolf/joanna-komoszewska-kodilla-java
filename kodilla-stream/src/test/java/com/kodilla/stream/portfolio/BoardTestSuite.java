...
class BoardTestSuite {
    ...
    @Test
    void testAddTaskListFindOutDateTasks(){
        //Given
        Board project = prepareTestData();
        //When
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        Board project = prepareTestData();
        //Then
        List<TaskList> taskListInProgressToAverage = new ArrayList<>();
        taskListInProgressToAverage.add(new TaskList("In progress"));

        double averageWorkingOnTask = project.getTaskLists().stream()
                .filter(taskListInProgressToAverage::contains)
                .flatMap(tl-> tl.getTasks().stream())
                .map(period -> Period.between(period.getCreated(), LocalDate.now()).getDays())
